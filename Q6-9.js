...
function selectQ(x){
    ...
    switch (detail){
        ...
        case 4:     // 로딩 화면
            ...
            if (elapsedTime >= 1) {       // 시간 지난 후 자동 넘어가도록
                detail = 5;
            }
            break;
...
function cursorChange(){
    ...
    if(state == 6){
        if (detail == 0 || detail == 1 || detail == 2 || detail == 3) {     //선택지 버튼
            if (x > b1x-147 && x < b1x+147 && y > b1y-20 && y < b1y+20) {
                onButton = true;
            } else if (x > b2x-147 && x < b2x+147 && y > b2y-20 && y < b2y+20) {
                onButton = true;
            } else if (x > b3x-147 && x < b3x+147 && y > b3y-20 && y < b3y+20) {
                onButton = true;
...
function selection(){
    ...
    if (mouseIsPressed){
        ...
        if (x > b4x - 41 && x < b4x + 41  && y > b4y - 17 && y < b4y+ 17){      //확인 버튼
            if (detail == 5) {
...
function Qselect(x){        // 선택할 경우 문제 + 선택된 선택지 화면 표시
